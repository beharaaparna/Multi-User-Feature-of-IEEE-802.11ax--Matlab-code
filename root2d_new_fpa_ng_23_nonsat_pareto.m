function F = root2d_new_fpa_ng_23_nonsat_pareto_dyn(x)
global W_ng;
global Nra_ng;
global m_ul;
global m_limit;
...
% delay2=7.5*(10^5);%((m+1)*timeq)/((x(1)^(m+1)))
delay=0;%(delay1+delay2)/2;
% qq=min(1,lambda*delay*(10^(-6)));
qq2=find_qq_pareto(K_scale,alpha,delay2*(10^(-6)));
qq1=find_qq_pareto(K_scale,alpha,delay1*(10^(-6)));
qq=(qq1+qq2)/2;
rr=find_rr_pareto(K_scale,alpha,timeq*(10^(-6)));%1-exp(-lambda*timeq*(10^(-6)));
den_val2=(1-qq)/rr;
if(x(1)~=0)
