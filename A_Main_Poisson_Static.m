...
global nra;global Nra_ng;global na_max;
...
global lambda;global afille;global buffer_max;

%% System Parameters given in Table.1
csi_flag=0; % Flag to enable Channel Sounding
twt_flag=0; % Flag to enable TWT
T_max=15;% TWT  t_Mean
flag_mimo=0;% Flag to enable MU-MIMO Transmissions
lcsi=0.1;% Channel Sounding Rate
ap_ant=1;% Number of AP Antennas
lambda=40;%Poisson Arrival rate
Nru=16;%Number of Resource Units
mcs=11;%Modulation & Coding Index
Nra=Nru;%Number of RA RUs
Nsa=Nru-Nra;%Number of SA RUs
grouping_ratio=0.5;%Percentage of STAs selected for MU-MIMO Transmissions
tf_cycles =10000;%Number of time-slots(TF-Cycles)
n_iter =10;%Number of Iterations Considered
m_limit=5;% Maximum Retry Limit
PayLoad = 12000;%payload in bits
W=64;%Minimum Contention Window Size
na=256;%Maximum number of packets that can be aggregated
m=5;m_ul=m;vs=1;%Maximum Back-off Multiplying factor
b=160;%Bandwidth in MHz
kmax=2000;k11=[0]; %% Other arrays 
kin1=10;
...
if(flag_mimo) % RUs dedicated for MU-MIMO transmissions
...
Vs=vs;an_st=vs; %Number of antennas at the STA
f_subcarrier=78.125;%Sub-carreir spacing
n_tones =1000*(b/(Nru*f_subcarrier));%Number of tones/subcarriers
an_ap=ap_ant;
if(n_tones <=106)
    ap_ant=1;an_ap=ap_ant;%If the number of tones are less than 106 subcarriers, MU-MIMO transmissions is prohibited
end

time_csi=0;
t_ofdm=16;%OFDM Symbol Duration
[r]=find_r_ap(Nru,b,vs,mcs,t_ofdm,f_subcarrier);%Datarate Calculation
[r_csi]=find_r_ap(74,b,vs,mcs,t_ofdm,f_subcarrier);%CSI Datarate
...
W_ul=W;
W_ng=W;W_g=W;
[cur_sim_na] = find_na_mu();%Finding teh actual number of packets that can be aggregated
na_max=cur_sim_na;
lim=tf_cycles+10;
...
for num_stationb=k1 %Number of STAs array
    drop_time_ng_isi=[];
    drop_time_g_isi=[];
    ...
    twt_stas=[];
    if(twt_flag)
        twt_stas=randi(T_max,1,num_stationb);%% selecting the STAs awake in the current time-slot as per the TWT
    end

    for sta_iter = 1:1:n_iter %%Iterations
        ...
        if(flag_mimo)
            vu=ceil(grouping_ratio*n); g=vu; %% Stations involved in teh MU-MIMO transmissions
        else
            vu=0;g=0;
        end
        act_stas=n;
        ...
        if(n~=0)
            ...
            %% Initial random Selection of backoff counter values for the different STAs
            for kkl=1:1:n
                if(kkl<=g)
            ...
            avg_pkt_buffer=[]; pkts_trans=[];len_cont_stas=[];pkt_buff_suc=[];
            n_old_pkts_array=[];

            %% Time-slot advancement

            for iteration = 1:tf_cycles %Iteration for each Time-slot
                lam=[];
                if(iteration  ~= 0)
                    cur_contending_stations=[];
                    lam = exprnd(1/lambda,[1,n]);% Generation of Inter-aarival rate of the packets as per the Poisson distribution
                    for qq=1:1:n
                        if(lam(qq) <= (timeq*(10^(-6))))
                            n_pkts_buff(qq)= n_pkts_buff(qq)+1; %Check if the STA has a packet in its buiffer memeory for the current time-slot
                        end
                    end
                    ...
                    for qq=1:1:n
                        if(n_pkts_buff(qq)>0)
                            nra=nra+1;
                            cur_contending_stations = [cur_contending_stations qq]; %Based on the buffer status of each of these STAs, calculate the actual number of contending STAs for the current time-slot.
                        end
                    end
                else
                    nra=n;
                ...
                RU_status = zeros(Nra,lim);%%These arrays are used to store the temporaruy status of the RUs
                Group_status= zeros(Nra,lim);
                Coll_status= zeros(Nra,lim);
                cur_OBO = OBO(cur_contending_stations);

                %% Back-off decreemntation Process
                for kkl=1:1:nra
                ...
                %% Finding the RUs in which sucecsful transmissions occured
                ...
                %% Finding the collision status of each RU
                jj=1;
                for i = 1:(Nra)
                ...
                %% Modifying teh contention window (CW) size and Retry limit (m) as per the collision/success status
                ...
                through_simu =[through_simu (packet*((Nsa*an_ap)+pkts_dummy))]; %% Throughput Calculation

                if(Nsa==0 && length(successful_tx_nodes)~=0)
                    nCycles_success_ra =nCycles_success_ra+1;
                end

                %% Handling the discarded nodes
                for kkl=1:1:length(successful_tx_nodes)
                    nodee=successful_tx_nodes(kkl);
                    name_name=nodee;%cur_contending_stations(nodee);
                    vv=n_pkts_buff(name_name);
                    pkt_buff_suc=[pkt_buff_suc vv];
                    pkts_trans=[pkts_trans min(na_max,vv)];
                    if(vv <= na_max)
                        n_pkts_buff(name_name)= 0;
                    else
                        n_pkts_buff(name_name)= 0;
                        n_old_pkts_array=[n_old_pkts_array n_pkts_buff(name_name)];
                    end
                end

                for kkl=1:1:length(disc_nodes)
                ...
                %% Logging the Intermediate Data to a File
                thr=(packet*na_curr*((Nsa*an_ap)+((mean(n_pkts_actual)))))/(timeq);
            ...
            end% end of one time-slot (tf cycle)
        end
    ...
    if(n~=0 )
        ...
        %% Throughput and RA-Capability Theory calculation
        if(csi_flag)
            bsr_simu=[bsr_simu factor(index)*mean(bsr)];
            throughput = [throughput factor(index)*(packet*cur_na*((Nsa*an_ap)+((mean(n_pkts_stas)))))/(timeq)];
            [th,ra]= func_new_fpa(m_limit,n,Nsa*an_ap); a_ng=gamma_nga/Nra_ng;% Calculation using Fixed Point method
            if(grouping_ratio==0)
            ...
            gh=length(pkts_trans);
            yh=ceil(gh*3/4);
            throughput = [throughput thr_simu*nanmean(pkts_trans(yh:gh))];
            [th,ra,n_pkts_suc,pkts_txted,gamma,delay1,delay2]= func_new_fpa_nonsat(m_limit,n,Nsa*an_ap);
            tau_g_array=[tau_g_array tau_ga];
            tau_ng_array=[tau_ng_array tau_nga];

            %% Number of Aggregated Packets Theory Calculation
            summ=lambda*delay1*(10^-6);
            summqq=0;
            for pp=1:1:2000
                valo1=exp(-lambda*delay1*(10^-6));
                valo2=(lambda*delay1*(10^-6))^pp;
                valo3=factorial(pp);
                valo=pp*valo1*valo2/valo3;
                if(pp>70)
                    if(isnan(valo))
                        break;
                    end
            ...
            gh=length(pkts_trans);
            yh=ceil(gh*0.98);
            na_simu= nanmean(pkts_trans(yh:gh));
            %%%%%%%%%%%%%%%%
        ...
        delayxng111=find_moments_mean_alt(W,m_limit,m,gamma_nga,Nra_ng);%mean(pc_ing)
    ...
    Analytical_Throughput= throughputa_fpa
    Simulation_Throughput= throughput
end
