function [th,ra,n_suc_pkts,pkts_txted,gamma_ng,delay1,delay2]= func_new_fpa_nonsat(m_limit,n,n_sa_stas)
global W_ng;
...
if(Nra_ng~=0)
    [tau_ng,gamma_ng]=find_new_fpa_ng_nonsat();
else
        tau_ng=0;gamma_ng=0;

end
[delay1,delay2]=find_moments_mean_alt_drop(W_ng,m_limit,m_ul,gamma_ng,Nra_ng);
[delay1,n_suc_pkts,pkts_txted]=find_moments_mean_alt_n_pkjts(W_ng,m_limit,m_ul,gamma_ng,Nra,delay1*(10^(-6)));
% n_drop_pkts=0;
% [n_suc_pkts,n_drop_pkts]=find_moments_mean_alt_drop2(W_ng,m_limit,m_ul,gamma_ng,Nra_ng);
tot_delay=(delay1+delay2)/2;
tau_nga=tau_ng;
if(an_ap~=1)
    if(Nra_g~=0)
